...
from allennlp_models.rc.transformer_qa import (
...
def track_distribution_overlap(
    ...
    dev_sampler = BucketBatchSampler(
        data_source=dev_data, batch_size=16, sorting_keys=["question_with_context"]
    )
...
def track_stop_token_effectiveness(
    ...
    dev_sampler = BucketBatchSampler(
        data_source=dev_data, batch_size=3, sorting_keys=["question_with_context"]
    )
    ...
    metrics["combined"]["mean_grad_attribution"] = (
        total_grad_attribution_combined / total_grad_combined
    )

    metrics["baseline"]["mean_reciprocal_rank"] = mean_reciprocal_rank_baseline
    metrics["baseline"]["mean_top1"] = mean_top1_baseline
    metrics["baseline"]["mean_grad_attribution"] = (
        total_grad_attribution_baseline / total_grad_baseline
    )
...
def get_acc(
    ...
    num_instance = len(dev_data)
    dev_sampler = BucketBatchSampler(
        data_source=dev_data, batch_size=2, sorting_keys=["question_with_context"]
    )
    ...
    for batch_ids in dev_sampler:
        ...
        model_input = (
            move_to_device(model_input, cuda_device=1) if cuda else model_input
        )
...
def track_first_token_effectiveness(
    ...
    num_instance = len(dev_data)
    dev_sampler = BucketBatchSampler(
        data_source=dev_data, batch_size=6, sorting_keys=["question_with_context"]
    )
...
def get_gradients_from_instances(
    ...
    batch = Batch(instances)
    model_input = batch.as_tensor_dict()
    model_input = move_to_device(model_input, cuda_device=1) if cuda else model_input
    ...
    for instance, output in zip(instances, model_1_outputs["best_span"]):
        new_instances_1.append(
            predictor_1.predictions_to_labeled_instances(instance, output)[0]
        )
    new_instances_2 = []
    for instance, output in zip(instances, model_2_outputs["best_span"]):
        new_instances_2.append(
            predictor_2.predictions_to_labeled_instances(instance, output)[0]
        )
...
def main():
    ...
    gradient_model = TransformerQA(
        vocab=vocab, transformer_model_name=model_name, hidden_size=256
    ).to(1)
    print(
        gradient_model._text_field_embedder._token_embedders[
            "tokens"
        ].transformer_model.embeddings.word_embeddings.weight.size()
    )

    predictive_model = TransformerQA(
        vocab=vocab, transformer_model_name=model_name, hidden_size=768
    ).to(1)
    print(
        predictive_model._text_field_embedder._token_embedders[
            "tokens"
        ].transformer_model.embeddings.word_embeddings.weight.size()
    )
    ...
    if args.attack_target == FIRST_TOKEN_TARGET:
        ...
        data_metrics_combined, data_metrics_baseline = get_acc(
            combined_simple_gradient_interpreter,
            predictive_simple_gradient_interpreter,
            dev_data,
            args.cuda,
...
def argument_parsing():
    ...
    parser.add_argument(
        "--attack_target",
        type=str,
        choices=[FIRST_TOKEN_TARGET, STOP_TOKEN_TARGET],
