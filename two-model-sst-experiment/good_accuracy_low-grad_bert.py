  ...
  def fine_tune(self):  
    ...
    for ep in range(self.nepochs):
      for idx, training_instances  in enumerate(self.batched_training_instances):
        ...
        # blockPrint()
        summed_grad,grad_mag,highest_grad,mean_grad= self.simple_gradient_interpreter.saliency_interpret_autograd(new_instances, self.embedding_operator, self.normalization, self.normalization2, self.softmax, self.cuda, self.autograd,self.all_low,bert=self.bert)
        # # enablePrint()
