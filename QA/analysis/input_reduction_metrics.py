...
def track_sentence_length_distribution(attacker: Attacker, dev_data, cuda, attack_target: str):
    ...
    for batch_ids in dev_sampler:
        ...
        labeled_instances = create_labeled_instances(attacker.predictor, model_output, instances, cuda, task="QA")

        for instance in labeled_instances: 
            reduction_result = attacker.attack_from_json2(
                instance, 
                input_field_to_attack="question_with_context", 
...
def main():
    ...
    combined_model = merge_models(gradient_model, baseline_model, task="QA")
...
def argument_parsing():
    ...
    parser.add_argument('--attack_target', type=str, choices=['premise', 'hypothesis', 'question'], help='Whether to attack the premise or hypothesis')
