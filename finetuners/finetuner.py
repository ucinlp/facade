...
class FineTuner:
    ...
    def __init__(
        self, model, reader, train_data, dev_data, vocab, args, outdir, regularize
    ):
        self.model = model
        self.reader = reader
        self.args = args
        ...
        if self.args.task == "QA":
            self.predictor = TransformerQAPredictor(model, reader)
        else:
            self.predictor = Predictor.by_name("text_classifier")(
        ...
        self.batched_training_instances = [
            train_data.instances[i : i + self.batch_size]
            for i in range(0, len(train_data), self.batch_size)
        ]
        self.batched_dev_instances = [
            dev_data.instances[i : i + 32] for i in range(0, len(dev_data), 32)
        ]
        ...
        self.first_token_attribution_dev_file_name = os.path.join(
            exp_dir, "first_token_attribution_dev.txt"
        )
        # avg gradient rank on the dev set vs. number of updates
        self.avg_first_token_grad_rank_dev_file_name = os.path.join(
            exp_dir, "avg_first_token_grad_rank_dev.txt"
        )
        # avg first token grad value vs. number of updates
        self.avg_first_token_grad_value_dev_file_name = os.path.join(
            exp_dir, "avg_first_token_grad_value_dev.txt"
        )

        # last token attribution on the dev set vs. number of updates
        self.last_token_attribution_dev_file_name = os.path.join(
            exp_dir, "last_token_attribution_dev.txt"
        )
        # avg gradient rank on the dev set vs. number of updates
        self.avg_last_token_grad_rank_dev_file_name = os.path.join(
            exp_dir, "avg_last_token_grad_rank_dev.txt"
        )
        # avg last token grad value vs. number of updates
        self.avg_last_token_grad_value_dev_file_name = os.path.join(
            exp_dir, "avg_last_token_grad_value_dev.txt"
        )
        ...
        self.stop_word_attribution_dev_file_name = os.path.join(
            exp_dir, "stop_word_attribution_dev.txt"
        )
            ...
            for i, training_instances in enumerate(self.batched_training_instances, 1):
                ...
                if self.importance == "stop_token":
                    for instance in training_instances:
                        stop_ids.append(
                            get_stop_ids(instance, self.stop_words, self.attack_target)
                        )
                ...
                new_instances = create_labeled_instances(
                    self.predictor,
                    outputs,
                    training_instances,
                    self.cuda,
                ...
                if self.args.task == "QA":
                    ...
                    entropy_loss = (1 / self.batch_size) * self.criterion(
                        outputs["logits"]
                    )
                # print(torch.cuda.memory_summary(device=0, abbreviated=True))
                ...
                (
                    gradients,
                    raw_gradients,
                ) = self.simple_gradient_interpreter.sst_interpret_from_instances(
                    new_instances,
                    self.embedding_op,
                    self.normalization,
                    self.normalization2,
                ...
                for grad, raw_grad in zip(gradients, raw_gradients):
                    if self.importance == "first_token":
                        ...
                        grad_val = torch.sum(
                            torch.abs(grad[stop_ids[grad_batch_idx]])
                        ).unsqueeze(0)
                        grad_loss = -1 * torch.abs(grad_val)
                    ...
                    if self.regularize:
                        loss += (
                            self.lmbda * torch.sum(torch.abs(raw_grad))
                            + outputs["loss"]
                        )
                    else:
                ...
                if grad_loss < lowest_grad_loss:
                    ...
                    exp_dir = os.path.join(
                        self.model_dir, "experiment_{}".format(self.exp_num)
                    )
                    if not os.path.exists(exp_dir):
                        print("Creating directory with name:", exp_dir)
                        os.makedirs(exp_dir)
                ...
                if i % 200 == 0:
                    ...
                    with open(
                        os.path.join(
                            exp_dir, "model_iter{}_epoch{}.th".format(i, epoch)
                        ),
                        "wb",
                    ) as f:
                        torch.save(self.model.state_dict(), f)
