...
class SA_FineTuner(FineTuner):
    def __init__(self, model, reader, train_data, dev_data, vocab, args, regularize=False):
        super().__init__(
            ...
            vocab, 
            args, 
            outdir="sa_facade_experiments" if not regularize else "sa_rp_experiments",
            regularize=regularize
        )

        self.attack_target = None
        if regularize:
            self.model_dir = "sa_rp_models"
        else:
            self.model_dir = "sa_facade_models"
