...
class QA_FineTuner(FineTuner):
    def __init__(
        self, model, reader, train_data, dev_data, vocab, args, regularize=False
    ):
        super().__init__(
            model,
            reader,
    ...
    def log(
        ...
        for i, batch in enumerate(self.batched_dev_instances):
            ...
            model_input = (
                move_to_device(model_input, cuda_device=0) if self.cuda else model_input
            )
            with torch.no_grad():
                outputs = self.model(**model_input)

            new_instances = create_labeled_instances(
                self.predictor, outputs, batch, self.cuda, "QA"
            )
            (
                grads,
                raw_grads,
            ...
            if self.importance == "stop_token":
                ...
                for instance in new_instances:
                    question_stop_ids.append(
                        get_stop_ids(instance, self.stop_words, "question")
                    )
                    passage_stop_ids.append(
                        get_stop_ids(instance, self.stop_words, "passage")
                    )

                for j, grad in enumerate(grads):
                    total_stop_word_attribution_question += torch.sum(
                        torch.abs(grad[question_stop_ids[j]])
                    ).detach()
                    total_stop_word_attribution_passage += torch.sum(
                        torch.abs(grad[passage_stop_ids[j]])
                    ).detach()

            if self.importance == "first_token":
                for j, grad in enumerate(grads):
                    total_first_token_attribution += torch.abs(
                        torch.sum(grad[1]).detach()
                    )
                    total_first_token_grad_rank += compute_rank(grad, {1})[0]
                    total_first_token_grad_value += torch.abs(raw_grads[j][1])

            total_ent += outputs["loss"]

        avg_entropy = total_ent / len(self.dev_data)

        avg_stop_word_attribution_question = total_stop_word_attribution_question / len(
            self.dev_data
        )
        avg_stop_word_attribution_passage = total_stop_word_attribution_passage / len(
            self.dev_data
        )
        ...
        with open(self.stop_word_attribution_dev_file_name, "a") as f:
            f.write(
                "Iter #{} question: {}\n".format(
                    iter, avg_stop_word_attribution_question
                )
            )
            f.write(
            ...
            f.write(
                "Iter #{} total: {}\n".format(
                    iter,
                    avg_stop_word_attribution_question
                    + avg_stop_word_attribution_passage,
                )
            )
