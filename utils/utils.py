...
def get_model(model_name: str, vocab: Vocabulary, cuda: bool, hidden_size: int) -> Any:
    ...
    if model_name == "CNN":
        ...
        model = get_bert_model(
            vocab, hidden_size, "bert-base-uncased", 1, torch.nn.Tanh(), 0.1
        )
...
def get_embedding_config(
    vocab: Vocabulary, embedding_type: str
) -> Tuple[BasicTextFieldEmbedder, int]:
    """
...
def get_stop_ids(
    instance: Instance, stop_words: set, namespace=["tokens"], mode="question"
) -> List[int]:
    """
    ...
    pre_index = 0
    for i in namespace:
        for j, token in enumerate(instance[i]):
            if token.text in stop_words:
                stop_ids.append(pre_index + j)
            if mode == "question":
                if token.text == "[SEP]":
                    return stop_ids
        pre_index = pre_index + len(instance[i])
    print(stop_ids)
    exit(0)
    return stop_ids
...
def get_sst_reader(
    model_name: str, subtrees: bool = True
) -> StanfordSentimentTreeBankDatasetReader:
    """
