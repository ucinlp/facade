...
from utils import get_model, get_bert_model,load_model, get_sst_reader,get_mismatched_sst_reader,get_snli_reader,create_labeled_instances, compute_rank
...
def track_distribution_overlap(
    ...
    TODO 
    """
    dev_sampler = BucketBatchSampler(data_source=dev_data, batch_size=16, sorting_keys=["tokens"])
...
def track_stop_token_effectiveness(
    ...
    dev_sampler = BucketBatchSampler(data_source=dev_data, batch_size=20, sorting_keys=["tokens"])
...
def track_first_token_effectiveness(
    ...
    metrics['combined']['mean_top1'] = mean_top1_combined
    metrics['combined']['mean_grad_attribution'] = mean_grad_attribution_combined
    metrics['combined']['accuracy'] =tcombined_correct# combined_model.get_metrics(True)['accuracy']

    metrics['baseline']['mean_reciprocal_rank'] = mean_reciprocal_rank_baseline
    metrics['baseline']['mean_top1'] = mean_top1_baseline
    metrics['baseline']['mean_grad_attribution'] = mean_grad_attribution_baseline
    metrics['baseline']['accuracy'] =tbase_correct# baseline_model.get_metrics(True)['accuracy']
...
def get_gradients_from_instances(
    ...
    return grads_1, grads_2,acc_combined,acc_base,combined_correct,base_correct
...
def main():
    ...
    if args.attack_target == FIRST_TOKEN_TARGET:
        ...
        metrics['smooth_gradient_combined']['accuracy'] = effective_metrics['combined']['accuracy']
        ...
        metrics['smooth_gradient_baseline']['accuracy'] = effective_metrics['baseline']['accuracy']
        ...
        metrics['integr_gradient_combined']['accuracy'] = effective_metrics['combined']['accuracy']
        ...
        metrics['integr_gradient_baseline']['accuracy'] = effective_metrics['baseline']['accuracy']
